...
(function($){
  ...
  $.fn.checkbox = function(options) {
    ...
    return this.each(function(){
      ...
      if($(ch).filter(':radio').length){settings.cls = 'jquery-checkbox-radio';}
      ch.wrapper = $('<div class="' + settings.cls + '"><div class="mark"></div></div>');
      ch.wrapperInner = ch.wrapper.children('div:eq(0)');
      ch.wrapper.hover(
        function(e){
      ...
      $ch.bind('disable', function(){
        ...
        if(label.length){
          label.addClass(settings.cls+'-label-disabled');
        }
      }).bind('enable', function(){
        ch.wrapperInner.removeClass(settings.cls+'-disabled');
        if(label.length){
          label.removeClass(settings.cls+'-label-disabled');
        }
      });
      $ch.bind('check', function(){
        $ch.triggerHandler('click');
        ch.wrapper.addClass(settings.cls+'-checked');
        if(label.length){
          label.addClass(settings.cls+'-label-checked');
        }
      }).bind('uncheck', function(){
        $ch.triggerHandler('click');
        ch.wrapper.removeClass(settings.cls+'-checked');
        if(label.length){
          label.removeClass(settings.cls+'-label-checked');
        }
      });
